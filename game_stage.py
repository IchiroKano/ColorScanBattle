import pygame
from game_ui import draw_button, draw_hp_bar, draw_step_label, draw_message_label
from game_player import draw_player_info
...
from game_ai import generate_live_commentary
...
class StageRenderer:
    ...
    def draw_result(self, screen, font, big_font, hp1, hp2, width, height,
                    ...
                    frame, jump_progress1, jump_progress2, step):
...
def draw_battle(screen, font, big_font, log_font, width, height, turn, step, hp1, hp2,
    ...
    if step == 1:
        ...
        result_label = big_font.render(result_text, True, (0, 0, 0))
    ...
    commentary_y = int(height * 0.9)
    commentary_surface = log_font.render(live_commentary, True, (0, 0, 0))
    screen.blit(commentary_surface, (commentary_x, commentary_y))

    # ログ表示（画面中央）
    log_y = int(height * 0.08) + log_font.get_height()
    for line in battle_log:  # ログを正順に表示
        log_surface = log_font.render(line, True, (50, 50, 50))
